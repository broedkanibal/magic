...
*Utkastet nedan beskriver `zon:'grav'`-spår och filtret i avstamBord. Det byggda föder inga spår i rutan (`matcha` slänger fynden där), så något `zon:'grav'` finns inte. Högens ändringar kommer i stället från högvakten (K10/K11, MES-85).*
...
**Byggt 2026-09-12 (MES-80)** med en avvikelse: referenserna ligger ALDRIG i den sparade poolposten — de bor i en egen post (`ref:<kod>`, samma objektlager) och vävs in i poolen i MINNET (`Ref.tillampa` → `Pool.laggTill`, `ORB.packTill`) när `byggLekPool` lämnar poolen och när en ny lärs. Därför behöver golden-poolen aldrig raderas: `kor.html` sätter `MESA_REF`/`MESA_LAR = false` på appens fönster utan `--ref`/`--lar-ref`, och `--glom-ref` tar bort ref-posten. Lärandet: `kamIdentifiera` minns senaste beskärningen per spår (`kamSistaBild`, 24 spår), `Kamera.svarAI` (ett kort, `hog`) och `Kamera.namnge` ropar `cb.lart` → `kamLart` → `Ref.lar`; en gång per spår och namn. Utåt heter en referens kortet (`refSid` i `identifyAt`, `kamIdentifiera`, `kamAiPoster`). `--lar-ref` lär facit efter domen bara för spår som inte redan var säkert rätt; `--lar-ref --ref` i följd är leave-one-out. Telefonens statusrad säger `N learned photos`; *Forget learned photos…* i lekens meny glömmer på datorn och skickar `glomref` till telefonen.

### K10/K11 — kort som läggs på graveyard-högen i bild går dit utan fråga (M) — BYGGD (MES-85)
Premissen i utkastet (spår med `zon:'grav'` ur K9-lite) fanns inte: rutan föder inga spår, så telefonen fick först en egen signal. Jespers beslut 2026-09-13:
- **B1 — högvakten** (`gravVakt` i Kamera): rutans celler jämförs med hur högen såg ut när den senast låg stilla i `stillaMs`. Minst `GRAV_ANDEL` (6 %) av cellerna ändrade (> `utseende`) med ett korts struktur → `gravN++`. En hand som sveper förbi eller vilar (slät) räknas inte. Inget namn läses, ingen AI. Rapporten och hjärtslaget bär `grav: { n, sen }` (null utan ruta). Bänken GY3–GY5.
- **B2 — auto med notis:** i Table leads (`pol.saknas`) går ett kort som försvann till graveyard när högen ändrades inom `[borta − GRAV_FORE 2,5 s, borta + GRAV_EFTER 5 s]` (`gravAutoOm` i `avstamBord`, `k.gravAuto`). Också efter nådatiden, om kortet redan tonats ned (efterskottet). Mattan flyger kortet till högen med `flygTillGrav`, lägger det överst och visar "X → graveyard" med ångra, som ger tillbaka frågan (`gravSagda` i `renderGrid`).
