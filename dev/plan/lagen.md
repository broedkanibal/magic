...
- `const pol = spelPolicy(mig)` överst; `tapSynk = pol.tap && kamGrund !== null`; `grundSteg` returnerar null när `!pol.grund` (nu, inte i MODE-1).
- `k.kam` skrivs i `lagg` och vid skapande när `t.w != null && !franHelbild(t) && tillstand ∈ {stilla, klar}` och mitten flyttat > `AUTO_FLYTT·t.w` (eller saknas); `nar` bumpas bara då. Skrivs i båda lägena (minne). `andrat = true` vid flytt.
- Screen leads: `fysisk`: `flyttaTill(grav/exil)`, `removeAt`, arket och `svaraLyftAlla` sätter `k.fysisk = true` när `k.spar != null` (utanför skivan; `borttagna.add(spar)` som `removeAt` redan gör 6843); i skivan: `ledigt()`/steg 3 får returnera `fysisk`-kort oavsett zon (tyst ombindning, zonen kvar), steg 4 räknar `fysisk` som på bordet; saknad-slingan i skarm: `delete k.spar/k.borta` tyst efter `BORTA_NAD` och `delete k.fysisk`; ingen `lyft`, ingen nådtimer. `sammaPlatsBorta(g)` (tackning ≥ `SAMMA_KORT_TACKNING` mot `k.kam`) binder ett borta-kort utan `fysisk` (efter omladdning) tyst; annars `aterFragor.set(t.id, {namn, cid, sedan})` → platshållare slag `'ater'` "<name> is back on the table? Return it to play / Not now" (`svaraAter` i skivan; nej → `borttagna`). `aterFragor` rensas i nollstall-grenen och när spåret dör.
- Besvärjelseregeln (båda lägena): kort skapat av kameran (`ny`-stämpel < 20 s) med typrad `/\b(Instant|Sorcery)\b/` (via stubbad `zonAv`-liknande hjälpare — `typLinje` 5644 måste stubbas i harnessarna om den används i skivan; alternativt sätt `k.typ` vid skapande utanför skivan) vars spår dör → `zon = 'grav'`, `k.spellAuto = nu`; toasten "Lightning Bolt resolved → graveyard · Undo" ritas av mattan.
- Steg 1 (15675): `lyft` bara när `pol.saknas`. `autoKortPaBordet` 15267 kräver `iSpel`.
- `autoPlatser` får `extra.ater` (ren); `platsHtml` ritar `'ater'` med `data-ater`; kanske också `'tak'` (D5-8).
- `kanLyftas(c)` i Table leads: falskt när `c.attachedTo`:s värd eller stackledaren (`gi 0`) har spår i `synliga` (täckta kort är inte borta).
- avstamning.cjs K1–K10 (skarm skriver aldrig tapped; spår dör utan nåd; grav-kort med levande spår → inget nytt; spår föds om på samma plats → tyst ombindning; `fysisk` + omfödsel på ny plats → tyst; utan `fysisk` → aterFragor; svaraAter ja/nej; bord: som förut; grundSteg null i skarm; besvärjelse → grav). `dubbletter.cjs --lage skarm|bord`.
- Verifiera: konsolrecept i båda lägena (se §9); `dev/kolla.sh`; `dubbletter.cjs --fall 07 --lage bord` byte-identisk.
...
### MODE-5 — positionsspegling i Table leads (M, två commits) — sist
- Commit 1 (skivan): `k.kam`-skrivningen finns från MODE-3; P1–P5-prov (sätts vid skapande stilla/klar, inte 'ny'; under `AUTO_FLYTT` ingen ändring; över → nytt `nar`; helbildsspår sätter inget; skarm sätter minne).
- Commit 2 (mattblocket, utanför skivan): `camera_setups.ruta` får `vand` (0/90/180/270) + `spegel` — ALLA tre skrivare (16533, 16556, ny) bär dem, läsaren 16590 och `normaliseraKamRad` 16551 återställer, 16938 nollar; "Rotate camera view"-kontroll i lägespanelen (fyra rotationer + spegel); `kamTillMatta(kam, bord, vand, spegel)` bevarar bildens proportioner (letterbox in i en kameraregion av brädet med marginal så klampen mot graveyard-högen aldrig slåss med kameran); mattans skala = "bordet i kortenheter" (`CW / median(t.kort)`), inte fasta 1060 px; `speglaKamPos(mig)` i mattans render: bara `spar != null && kam && !lyft && paMattan`, `kamRitad !== kam.nar`, klamp, `flyttas`-stämpel → `.kamflytt` transition .32 s; anropas från `avstamBord`:s `andrat`-väg via `renderGrid` (utanför `medFjarr`, annars droppas `save()`); positionssparningar koalesceras ≥1 s i `sparaNu` när bara x/y ändrats.
- Drag av när `mittLage()==='bord' && e.spar != null && kamAnsluten`: pekargesten panorerar; släpp utan pan → chip "The table places this card · switch to Screen leads to move it". Platshållare i Table leads ritas vid spårets mitt.
- Mät: `kor.cjs` får "position updates on a still board per minute" (ska vara 0 i 01–06/08) och medianfel i kortbredder mot facits rutor (01/02/08); K5 (viloläge med hysteres) måste finnas först.
- Verifiera: konsol: spår x .2/.3 → kortmitt vid motsvarande brädpunkt; x .21 → ingen flytt; x .6 → glider .32 s; 'ny' → ingen flytt; motståndaren ser flytten; pointerdown → chip, ingen drag; efter nedtoning → drag igen; Screen leads → `slotBeside`, aldrig flytt.
