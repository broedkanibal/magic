...
// och hur många nya lås som föddes större än så eller vid bildens kant.
...
function kor(app, f) {
  ...
  for (const b of f.bordLogg) {
    ...
    las = s.las;
    if (s.ny) { ut.nya++; if (stor(las.box)) ut.stora++; if (vidKant(las.box)) ut.kanten++; }
  }
  return ut;
}
