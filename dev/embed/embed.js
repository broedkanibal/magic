/* ══════════════════════════════════════════════════════════════════
   ...
   centralt (dev/embed/forrakna.cjs → tabellen card_embeddings) i stället
   för på varje telefon. byggLek frågar window.EmbedForrakade(MODELL, ids)
   efter det som inte redan ligger i IndexedDB och räknar själv bara resten.
   Funktionen sätts av appen (index.html, via Moln) och svarar med en Map
   id → Uint8Array (8 × 512 fp16, little-endian). Saknas den, är man
   offline eller saknas tabellen: allt räknas lokalt som förut.
   ══════════════════════════════════════════════════════════════════ */
(function (global) {
  'use strict';

  const V = 1;                               // receptets version: ändras den byggs allt om
  const MODELL = 'mobileclip-s0|v' + V;      // nyckeln i lagringen, tillsammans med Scryfall-id
  ...
  /* En öppning per sida, inte en per post: en Commander-lek är 900 poster. */
  let dbP = null;
  ...
  /* Förräknade vektorer: 8 × 512 fp16 (little-endian) → Float32Array, varje
     ...
     storleken inte stämmer — då räknas bilden lokalt. */
  const PER = ROTAR.length * VARIANTER.length;
  function franF16(bytes) {
    if (!bytes || bytes.byteLength !== PER * DIM * 2) return null;
    const dv = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength), ut = new Float32Array(PER * DIM);
    for (let i = 0; i < PER * DIM; i++) {
      const h = dv.getUint16(i * 2, true), s = h & 0x8000 ? -1 : 1, e = (h >>> 10) & 0x1f, m = h & 0x3ff;
      ut[i] = e === 0 ? s * m * 2 ** -24 : e === 31 ? 0 : s * (1 + m / 1024) * 2 ** (e - 15);
    }
    for (let j = 0; j < PER; j++) { let n = 0; for (let k = 0; k < DIM; k++) n += ut[j * DIM + k] ** 2; n = Math.sqrt(n) || 1; for (let k = 0; k < DIM; k++) ut[j * DIM + k] /= n; }
    return ut;
  }
  const medTak = (p, ms) => Promise.race([p, new Promise((_, rej) => setTimeout(() => rej(new Error('tidstak')), ms))]);

  /* Bygg inbäddningarna för en lek. kort: [{id, name, small|normal|bild}] — samma lista som
     byggPoolAv får. Svar: posten (sparad i IndexedDB under kod).
     onProg({done, total, fel, nya, hamtade}): nya = räknade här, hamtade = förräknade.
     o.forrakade: hämtaren (ersätter window.EmbedForrakade; null = räkna allt lokalt). */
  async function byggLek(kod, kort, o) {
    o = o || {};
    /* Modellen laddas medan lagringen tillfrågas; behövs den inte för bygget
       (allt fanns redan) väntar bygget inte på den — identifiera laddar den. */
    ...
    if (saknas.length && typeof hamtare === 'function') {
      try {
        const svar = await medTak(Promise.resolve(hamtare(MODELL, saknas)), o.hamtaMs || 60000);
        for (const [id, bytes] of (svar instanceof Map ? svar : new Map(svar || []))) {
          const vek = franF16(bytes); if (!vek) continue;
          const post = { nyckel: nyckelAv(id), vek, kalla: 'forrakad' };
          await satt('kort', post); sparade.set(String(id), post); hamtade++;
        }
      } catch (e) { /* lagringen svarade inte: resten räknas här */ }
    }
    if (kort.some(c => !sparade.has(String(c.id)))) await laddning;
    for (const c of kort) {
      const nyckel = nyckelAv(c.id);
      ...
      if (o.onProg) o.onProg({ done: ++done, total: kort.length, fel, nya, hamtade });
    }
